...
export const unblockOwner = async (req, res, next) => {
  try {
    ...
    res.status(500).json({ message: 'Error unblocking', error });
  }
};
...
export const blockOwner = async (req, res, next) => {
  try {
    ...
    // Check if the user is already blocked
    if (owner.isBlocked) {
      return res.status(409).json({ message: 'Owner is already blocked.' });
    }
    ...
    return res.status(200).json({ message: 'User blocked successfully.', owner });

  } catch (error) {
    res.status(500).json({ message: 'Error blocking owner', error });
  }
};
